...
int main(int argc, char **argv)
{
	...
	*/
	...
	pthread_create(&pth,NULL,threadFunc,(void *)virtual_base);
	...
	while(exit_flag == 0)
	{
		...
		//if(!freezeMain){
			...
			for(j = 0; j<8; j++){
				...
				/*int32_t output1 = alt_read_word(h2p_lw_quad_addr[0]);
				int32_t output2 = alt_read_word(h2p_lw_quad_addr[1]);
				int32_t output3 = alt_read_word(h2p_lw_quad_addr[2]);
				int32_t output4 = alt_read_word(h2p_lw_quad_addr[3]);
				int32_t output5 = alt_read_word(h2p_lw_quad_addr[4]);
				int32_t output6 = alt_read_word(h2p_lw_quad_addr[5]);
				int32_t output7 = alt_read_word(h2p_lw_quad_addr[6]);
				int32_t output8 = alt_read_word(h2p_lw_quad_addr[7]);*/

				int32_t arm_encoders1 = alt_read_word(h2p_lw_quad_addr_external[0]);
				int32_t arm_encoders2 = alt_read_word(h2p_lw_quad_addr_external[1]);
				int32_t arm_encoders3 = alt_read_word(h2p_lw_quad_addr_external[2]);
				int32_t arm_encoders4 = alt_read_word(h2p_lw_quad_addr_external[3]);
				...
				//if(0){
					...
					//printf("Motor Encoders: %d,%d,%d,%d,%d,%d,%d,%d\n",output1,output2,output3,output4,output5,output6,output7,output8);
...
void *threadFunc(void *virtual_base)
{
//-----------------------------------------------------------------------------------
//***********************************************************************************
//-----------------------------------------------------------------------------------
// Reading external encoders in thread function in order to send them to the python side.
// This will be removed later once control is done internally on the fpga
...
--------------------------------*/
	...
	while(system_state == 1 && socket_error == 0 ){
		...
		//sprintf(write_buffer,"nn %d %d %d %d qq",test,test,test,test);
